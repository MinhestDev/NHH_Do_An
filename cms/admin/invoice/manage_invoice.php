...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h5 class="card-title"><?php echo isset($id) ? "Cập nhật hóa đơn - ".$invoice_code : 'Tạo mới hóa đơn' ?></h5>
    </div>
    <div class="card-body">
                ...
                <div class="col-md-12">
                    <fieldset class="border-bottom border-info">
                        <div class="row">
                            <div class="form-group col-sm-4">
                                <label for="client_id" class="control-label text-info">Khách hàng</label>
                                <select name="client_id" id="client_id" class="custom-select custom-select-sm rounded-0 select2" data-placeholder="Chọn khách hàng" required>
                    ...
                    <fieldset class="border-bottom border-info">
                        ...
                        <div class="row align-items-end">
                            <div class="form-group col-sm-4">
                                <label for="service_id" class="control-label text-info">Dịch vụ</label>
                                <select id="service_id" class="custom-select custom-select-sm rounded-0 select2" data-placeholder="Chọn dịch vụ ở đây">
                            ...
                            <div class="form-group col-sm-4">
                               <button class="btn btn-flat btn-primary btn sm" type="button" id="add_to_list"><i class="fa fa-plus"></i> Thêm danh sách</button>
                            </div>
                        </div>
                        <table class="table table-hover table-striped table-bordered" id="service-list">
                            ...
                            <thead>
                                <tr class="bg-lightblue text-light">
                                    ...
                                    <th class="px-2 py-2 text-center">Mô tả</th>
                                    <th class="px-2 py-2 text-center">Giá</th>
                                </tr>
                            </thead>
                            <tbody>
                            <?php 
                            ...
                            <tfoot>
                                <tr class="bg-lightblue text-light disabled">
                                    ...
                                    <th class="px-2 py-2 text-right sub_total"><?php echo number_format($total,0) ?></th>
                                </tr>
                                <tr class="bg-lightblue text-light disabled">
                                    <th class="px-2 py-2 text-right" colspan="3">
                                        Mã giảm giá
                                    ...
                                    <th class="px-2 py-2 text-right discount"><?php echo isset($discount) ? number_format($discount,0) : "0.00" ?></th>
                                </tr>
                                <tr class="bg-lightblue text-light disabled">
                                    <th class="px-2 py-2 text-right" colspan="3">
                                        Tax <small><i>(Bao gồm)</i></small>
                                ...
                                <tr class="bg-lightblue text-light disabled">
                                    ...
                                    <th class="px-2 py-2 text-right grand_total"><?php echo isset($total_amount) ? number_format($total_amount,0) : "0.00" ?></th>
                                </tr>
                            </tfoot>
                        </table>
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label for="remarks" class="control-label text-info">Nhận xét</label>
                            ...
                            <?php if(isset($status)): ?>
                            <div class="form-group col-md-6">
                                <label for="status" class="control-label text-info">Trạng thái thanh toán</label>
                                <select name="status" id="status" class="custom-select selevt">
                                    <option value="0" <?php echo isset($status) && $status == 0 ? 'selected' : '' ?>>Đang thanh toán</option>
    ...
    <div class="card-footer text-center">
        <button class="btn btn-flat btn-sn btn-primary" type="submit" form="invoice-form">Lưu</button>
        <a class="btn btn-flat btn-sn btn-dark" href="<?php echo base_url."admin?page=invoice" ?>">Hủy</a>
    </div>
</div>
        ...
        $('#invoice-form').submit(function(e){
            ...
            var _this = $(this)
             ...
             if($('#service-list tbody tr').length <= 0){
                 alert_toast("Hãy thêm ít nhất 1 dịch vụ vào danh sách.","warning")
                 return false;
             }
			start_loader();
			$.ajax({
