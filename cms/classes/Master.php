...
Class Master extends DBConnection {
	...
	function save_client(){
		...
		if($resp['status'] == 'success'){
			if(empty($id)){
				$this->settings->set_flashdata('success'," Thêm thành công.");
			}else{
				$this->settings->set_flashdata('success'," Cập nhật thành công.");
			}
		}
	...
	function reset_password(){
		...
		if($update){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Lấy lại mật khẩu thành công.");
		}else{
			$resp['status'] = 'failed';
		...
		if(md5($_POST['cur_password']) == $this->settings->userdata('password')){
			...
			if($update){
				...
				if($resp->status == 'success'){
					...
					$this->settings->set_flashdata('success',"Thông tin đã cập nhật thành công.");
